...
def build_comparison_csv(qc_raw: dict, qc_cleaned: dict, output_dir: str, signal_type: str = "EEG") -> str:
    """
    qc_raw / qc_cleaned: the `result` dicts returned by a raw and cleaned pass
    of the SAME agent (EEG QC agent or ECG agent) for the same recording.
    signal_type: "EEG" or "ECG" — tags each row so both signal types can share
    one export file without their rows getting confused (their check_name
    sets are different, so this also makes filtering easy in Power BI).
    """
